...
function A:BuildUI(parent)
  ...
  local function row(lbl,key,y)
    ...
    local edit=CreateFrame('EditBox', nil, p, 'InputBoxTemplate') edit:SetSize(40,24) edit:SetPoint('LEFT', t, 'RIGHT', 10, 0) edit:SetNumeric(true) edit:SetNumber(GT.gdb.permissions[key] or 1)
    ...
    save:SetScript('OnClick', function() if not U:HasPermission(GT.gdb.permissions.adminMinRank) then UIErrorsFrame:AddMessage('Only Admin rank can change permissions',1,0,0) return end GT.gdb.permissions[key]=edit:GetNumber() GT.gdb.dataVersion=(GT.gdb.dataVersion or 1)+1 if GT.Comm then GT.Comm:BroadcastFull() end end)
  end
