...
-- CHANGED: no raid size or role caps; unlimited signups.
local function createEvent(title, ts, instance)
  ...
  local e = {
    ...
    roles = {},     -- kept as a table for future metadata; no caps enforced
    signups = {},
    comp = {},
  }
  GT.db.events[id] = e
...
function R:BuildUI(parent)
  ...
  -- REMOVED: Roles (T/H/D) input controls; unlimited signups (no caps)

  local createBtn = CreateFrame('Button', nil, p, 'UIPanelButtonTemplate')
  createBtn:SetSize(120,24); createBtn:SetPoint('LEFT', instanceDrop, 'RIGHT', 10, 0); createBtn:SetText('Create')
...
function Comp:Refresh()
  ...
  for name,s in pairs(e.signups or {}) do
    ...
    -- CHANGED: composition groups no longer depend on size; default to 5 groups.
    UIDropDownMenu_Initialize(dd, function(self, level)
      ...
      add('Unassigned', nil) local max=5 for g=1,max do add('Group '..g,g) end
    end)
    y=y-24
  end
...
function Comp:RefreshRight()
  ...
  -- CHANGED: fixed to 5 groups now that size is removed
  local max=5 local y=-8
  for g=1,max do local header=right:CreateFontString(nil,'OVERLAY','GameFontNormal') header:SetPoint('TOPLEFT',8,y) header:SetText('Group '..g) y=y-18
...
function R:Refresh()
  ...
  for _,e in ipairs(getSortedEvents()) do
    ...
    -- CHANGED: remove "%d-man" since size is gone
    title:SetText(string.format('%s  |  %s  |  %s', e.title, e.instance, date('%b %d %H:%M', e.ts)))

    -- CHANGED: show raw counts only (no caps)
    ...
    if U:HasPermission(GT.db.permissions.raidsMinRank) then
      ...
      del:SetScript('OnClick', function() StaticPopupDialogs['GTR_DEL_EVT']={ text='Delete this event?', button1='Yes', button2='No', timeout=0, whileDead=true, hideOnEscape=true, OnAccept=function() GT.db.events[e.id]=nil if GT.Comm then GT.Comm:Send('EVENT_DELETE', U:Serialize({id=e.id})) end if Log then Log:Add('INFO','EVENT','Deleted event '..e.id) end R:Refresh() end } StaticPopup_Show('GTR_DEL_EVT') end)
    end
