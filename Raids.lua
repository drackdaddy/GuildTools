...
local function createEvent(title, ts, instance)
  ...
  local e = {
    id       = id,
    title    = title or 'Raid',   -- we set this to the selected Raid name
    ts       = ts or time(),
...
function R:BuildUI(parent)
  ...
  -- Create button on its own line below header
  ...
  local function to12h(h24)
    local ap = (h24 < 12) and "AM" or "PM"
    local h12 = h24 % 12; if h12 == 0 then h12 = 12 end
    return h12, ap
  end

  local function to24h(h12, ap)
    if ap == "PM" then
  ...
  -- Day cells (7x6) with a dedicated selection overlay
  fly.cells = {}
  ...
  for i=1,42 do
    ...
    b:SetScript('OnClick', function(self)
      ...
      setSelectedByIndex(self.index) -- ensures ONLY this one is highlighted
    end)
  end

  prevBtn:SetScript('OnClick', function()
    current.month = current.month - 1
    if current.month == 0 then current.month = 12; current.year = current.year - 1 end
    refreshCalendar()
  end)
  nextBtn:SetScript('OnClick', function()
    current.month = current.month + 1
    if current.month == 13 then current.month = 1; current.year = current.year + 1 end
    refreshCalendar()
  end)
  ...
  hourEdit:SetPoint('LEFT', timeLbl, 'RIGHT', 8, 0)
  ...
  local minEdit = CreateFrame('EditBox', nil, timeRow, 'InputBoxTemplate')
  minEdit:SetSize(36,22)
  minEdit:SetPoint('LEFT', colon, 'RIGHT', 4, 0)
  ...
  ampmDrop:SetPoint('LEFT', minEdit, 'RIGHT', -16, 0)
  UIDropDownMenu_SetWidth(ampmDrop, 70)

  local ampmValue = "PM" -- default (20:00)
  ...
  applyBtn:SetScript('OnClick', function()
    ...
    sel.hour, sel.min = to24h(h12, ampmValue or "AM"), mm
    dateBtn:SetText(fmtBtn())
    fly:Hide()
  end)
  ...
  -- Seed once
  refreshCalendar()
  local initH12, initAP = to12h(sel.hour)
  hourEdit:SetText(string.format('%02d', initH12))
  ...
  -- >>> Forward declaration so Version callback can call it safely
  local function PopulateInstanceDrop(versionName) end

  -- Version dropdown (populate and wire callback)
  UIDropDownMenu_Initialize(versionDrop, function(self, level)
    for _, ver in ipairs({ "Classic", "Burning Crusade" }) do
      ...
      info.func  = function()
        ...
        PopulateInstanceDrop(selectedVersion)  -- safe, already declared
      end
      UIDropDownMenu_AddButton(info)
    end
  end)
  ...
  -- Define after instanceDrop exists (uses instanceDrop inside)
