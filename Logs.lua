...
function UI:BuildSyncWidget(parent)
  local box = CreateFrame('Frame', nil, parent, 'InsetFrameTemplate3') box:SetPoint('TOPLEFT',20,-60) box:SetPoint('BOTTOMRIGHT', -20, 20)
  local lbl = parent:CreateFontString(nil,'OVERLAY','GameFontNormal') lbl:SetPoint('TOPLEFT',20,-30) lbl:SetText('Recent Sync Activity')
  local scroll=CreateFrame('ScrollFrame','GTL_SyncScroll', box, 'UIPanelScrollFrameTemplate') scroll:SetPoint('TOPLEFT',10,-10) scroll:SetPoint('BOTTOMRIGHT', -30, 10)
  local content=CreateFrame('Frame', nil, scroll) content:SetSize(1,1) scroll:SetScrollChild(content) parent.syncContent=content
  local function refresh() local c=parent.syncContent for _,ch in ipairs({c:GetChildren()}) do ch:Hide(); ch:SetParent(nil) end local y=-2 local logs=Log:GetAll({cat='SYNC'}) local lastN=100 local start=math.max(1,#logs-lastN+1) for i=start,#logs do local e=logs[i] local line=string.format('|cffaaaaaa%s|r |cff00ff00[%s]|r %s', date('%H:%M:%S', e.ts), e.level, e.msg) local fs=c:CreateFontString(nil,'OVERLAY','GameFontHighlightSmall') fs:SetPoint('TOPLEFT',6,y) fs:SetJustifyH('LEFT') fs:SetText(line) y=y-14 end c:SetHeight(-y+10) end
  parent.SyncRefresh=refresh refresh() Log:Register(function(e) if e.cat=='SYNC' and parent.SyncRefresh then parent:SyncRefresh() end end)
end