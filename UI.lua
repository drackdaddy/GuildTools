...
function UI:Build()
  ...
  f.title:SetText('|cff48c9b0GuildTools|r')
...
function UI:SelectTab(i)
  ...
  for j=1,#UI.TABS do
    if j==i then f.pages[j]:Show(); PanelTemplates_SelectTab(f.tabs[j])
    else f.pages[j]:Hide(); PanelTemplates_DeselectTab(f.tabs[j]) end
  end
end
