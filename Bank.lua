...
function B:BuildUI(parent)
  ...
  send:SetScript('OnClick', function()
    ...
    GT.gdb.bankRequests[id] = req GT.gdb.dataVersion = (GT.gdb.dataVersion or 1) + 1
    if GT.Comm then GT.Comm:Send('BANK_UPDATE', U:Serialize(req)) end if Log then Log:Add('INFO','BANK','Created request '..(item:GetText() or 'Item')..' x'..qty:GetNumber()) end B:Refresh()
  end)
...
local function sorted() local a={} for _,r in pairs(GT.gdb.bankRequests or {}) do a[#a+1]=r end table.sort(a,function(x,y) return x.id<y.id end) return a end
...
function B:Refresh()
  ...
  for _,r in ipairs(sorted()) do
    ...
    if U:HasPermission(GT.gdb.permissions.bankMinRank) then
      local approve=CreateFrame('Button', nil, box, 'UIPanelButtonTemplate') approve:SetSize(80,22) approve:SetPoint('BOTTOMRIGHT', -90, 8) approve:SetText('Approve') approve:SetScript('OnClick', function() r.status='APPROVED'; GT.gdb.dataVersion=(GT.gdb.dataVersion or 1)+1; if GT.Comm then GT.Comm:Send('BANK_UPDATE', U:Serialize(r)) end; if Log then Log:Add('INFO','BANK','Approved '..(r.itemLink or 'item')) end; B:Refresh() end)
      local deny=CreateFrame('Button', nil, box, 'UIPanelButtonTemplate') deny:SetSize(80,22) deny:SetPoint('BOTTOMRIGHT', -5, 8) deny:SetText('Deny') deny:SetScript('OnClick', function() r.status='DENIED'; GT.gdb.dataVersion=(GT.gdb.dataVersion or 1)+1; if GT.Comm then GT.Comm:Send('BANK_UPDATE', U:Serialize(r)) end; if Log then Log:Add('INFO','BANK','Denied '..(r.itemLink or 'item')) end; B:Refresh() end)
    end
    y=y-74
  end
